...
export default function RedGiantRoom(props) {
    ...
    useEffect(() => {
        axiosWithAuth()
            .get(`/api/adv/rooms?planet=Ragnarok`)
            .then(res => {  
                let newRoom = props.currentRoom(res.data)              
                setPlanet(res.data)
                setCurrentRoom(newRoom)
                axiosWithAuth().post('/api/adv/changeplanet', {planet: 'Ragnarok', roomId: newRoom.id}).then(res => console.log(res)).catch(err => console.log(err.response))
