...
export default function Earth2Room(props) {
    ...
    useEffect(() => {
        axiosWithAuth()
            .get(`/api/adv/rooms?planet=Azeroth`)
            .then(res => {  
                let newRoom = props.currentRoom(res.data)              
                setPlanet(res.data)
                setCurrentRoom(newRoom)
                axiosWithAuth().post('/api/adv/changeplanet', {planet: 'Azeroth', roomId: newRoom.id}).then(res => console.log(res)).catch(err => console.log(err.response))
