...
export default function BrownExoRoom(props) {
    ...
    useEffect(() => {
        axiosWithAuth()
            .get(`/api/adv/rooms?planet=Europa`)
            .then(res => {  
                let newRoom = props.currentRoom(res.data)              
                setPlanet(res.data)
                setCurrentRoom(newRoom)
                axiosWithAuth().post('/api/adv/changeplanet', {planet: 'Europa', roomId: newRoom.id}).then(res => console.log(res)).catch(err => console.log(err.response))
